...
const i18n = {
    pl: {
        ...
        nav_budget: "Budżet", nav_expenses: "Wydatki", nav_fuel: "Paliwo", nav_rates: "Kursy", nav_journal: "Dziennik", nav_list: "Lista", nav_pdf: "PDF",
...
// --- STATE ---
const STORAGE_KEY = 'travel_budget_app_v13';
const DEFAULT_STATE = {
    ...
    currency: { manualRates: { 'PLN': 1, 'EUR': 4.3, 'USD': 3.9, 'GBP': 5.0, 'CHF': 4.5 } },
...
const generatePDF = () => {
    ...
    let html = `
        <div class="print-header">
            <h1 style="margin:0; color:#29606D;">${state.trip.name || t.title}</h1>
            <p style="margin:5px 0; font-weight:bold; color:#64748b;">${state.trip.startDate || ''} — ${state.trip.endDate || ''}</p>
        </div>
    `;
    ...
    if (getEl('pdf-inc-budget-check')?.checked) {
        html += `
            <div class="print-section">
                <h2>${t.pdf_inc_budget}</h2>
                <div class="print-grid">
                    <div class="stat-box"><strong>${t.total_budget}:</strong> ${state.trip.totalBudget} ${state.trip.baseCurrency}</div>
                    <div class="stat-box"><strong>${t.days}:</strong> ${calculateDays()}</div>
                    <div class="stat-box"><strong>${t.daily}:</strong> ${(state.trip.totalBudget / (calculateDays() || 1)).toFixed(2)} ${state.trip.baseCurrency}</div>
                </div>
                <h3>${t.planned_cats}</h3>
                <table>
                    <thead><tr><th>${t.category}</th><th>${t.amount}</th></tr></thead>
                    <tbody>
                        ${state.categories.map(c => `<tr><td>${c.name}</td><td>${c.plannedAmount} ${state.trip.baseCurrency}</td></tr>`).join('')}
                    </tbody>
                </table>
            </div>
        `;
    ...
    if (getEl('pdf-inc-expenses-check')?.checked && state.expenses.length > 0) {
        html += `
            <div class="print-section">
                <h2>${t.pdf_inc_expenses}</h2>
                <table>
                    <thead><tr><th>${t.when}</th><th>${t.title_label}</th><th>${t.category}</th><th>${t.amount}</th></tr></thead>
                    <tbody>
                        ${state.expenses.map(e => `<tr><td>${e.date}</td><td>${e.title}</td><td>${e.category}</td><td>${e.amountInBase.toFixed(2)} ${state.trip.baseCurrency} (${e.amount} ${e.currency})</td></tr>`).join('')}
                    </tbody>
                </table>
            </div>
        `;
    ...
    if (getEl('pdf-inc-fuel-check')?.checked) {
        const refuelTotal = state.fuel.refuelList.reduce((acc, x) => acc + (x.amount * x.fxRate), 0);
        const tollsTotal = state.fuel.tollsList.reduce((acc, x) => acc + (x.amount * x.fxRate), 0);
        html += `<div class="print-section"><h2>${t.pdf_inc_fuel}</h2>`;
        if (state.fuel.refuelList.length > 0) {
            html += `<h3>${t.refuel_title}</h3><table><thead><tr><th>${t.when}</th><th>Nazwa</th><th>${t.amount}</th></tr></thead><tbody>`;
            html += state.fuel.refuelList.map(r => `<tr><td>${r.date}</td><td>${r.name}</td><td>${(r.amount * r.fxRate).toFixed(2)} ${state.trip.baseCurrency}</td></tr>`).join('');
            html += `</tbody></table>`;
        }
        if (state.fuel.tollsList.length > 0) {
            html += `<h3>${t.nav_fuel}</h3><table><thead><tr><th>${t.when}</th><th>Nazwa</th><th>${t.amount}</th></tr></thead><tbody>`;
            html += state.fuel.tollsList.map(r => `<tr><td>${r.date}</td><td>${r.name}</td><td>${(r.amount * r.fxRate).toFixed(2)} ${state.trip.baseCurrency}</td></tr>`).join('');
            html += `</tbody></table>`;
        }
        html += `</div>`;
    }
    ...
    if (getEl('pdf-inc-list-check')?.checked && state.checklist.length > 0) {
        html += `<div class="print-section"><h2>${t.pdf_inc_list}</h2>`;
        const grps = [...new Set(state.checklist.map(i => i.group))];
        grps.forEach(g => {
            html += `<h4>${g}</h4><ul>`;
            html += state.checklist.filter(i => i.group === g).map(i => `<li>[${i.checked ? 'X' : ' '}] ${i.text}</li>`).join('');
            html += `</ul>`;
        });
        html += `</div>`;
    }
    ...
    if (getEl('pdf-inc-journal-check')?.checked) {
        html += `<div class="print-section"><h2>${t.pdf_inc_journal}</h2>`;
        Object.keys(state.journal.dayTitles).sort().forEach(k => {
            ...
            if (dayEntries.length > 0 || state.journal.dayTitles[k]) {
                html += `<div class="journal-day"><strong>${t.months[m]}, ${t.week} ${w}, ${t.weekdays[d]}</strong><br><em>${state.journal.dayTitles[k] || ''}</em>`;
                dayEntries.forEach(ek => {
                    html += `<div style="font-size:9pt; margin-top:3px;">${ek.split('-')[3].padStart(2, '0')}:00 — ${state.journal.entries[ek]}</div>`;
                });
                html += `</div>`;
            }
        });
...
const render = () => {
    ...
    ['budget', 'expenses', 'fuel', 'rates', 'journal', 'list', 'pdf'].forEach(id => {
        const btn = getEl(`nav-${id}`); if(btn) btn.textContent = t[`nav_${id}`];
    });
    ...
    ['exp-currency', 'fuel-currency', 'toll-currency', 'refuel-currency', 'conv-from'].forEach(id => {
        const sel = getEl(id); if(sel) { const v = sel.value; sel.innerHTML = options; if(v) sel.value = v; else sel.value = state.trip.baseCurrency; }
    });
    ...
    list.innerHTML = state.categories.map((c, i) => {
        ...
        return `<div class="category-block">
            <div class="list-item" style="border:none;">
                <div style="flex:1;"><div style="font-weight:700">${c.name}</div><input type="text" class="cat-amt-input" data-index="${i}" value="${c.plannedAmount}" style="width:80px;"> <small>${state.trip.baseCurrency}</small></div>
            ...
            <div class="cat-progress-container">
                ...
                <div class="cat-progress-label"><span>Wydano: ${spent.toFixed(2)}</span><span>${progress.toFixed(0)}%</span></div>
            </div>
        </div>`;
    }).join('');
};
...
const renderExpenses = () => {
    ...
    if (list) list.innerHTML = state.expenses.map(e => `<div class="list-item">
        <div class="list-item-main"><div class="list-item-title">${e.title}</div><div class="list-item-sub">${e.date} • ${e.category}</div></div>
        <div class="list-item-values"><div class="list-item-base">${e.amountInBase.toFixed(2)} ${state.trip.baseCurrency}</div></div>
        <button class="btn btn-outline btn-sm remove-exp" data-id="${e.id}">×</button>
    </div>`).join('');
};

const renderFuelAndTransport = () => {
    const cost = ((state.fuel.distanceKm * state.fuel.consumptionLPer100) / 100) * state.fuel.fuelPricePerL * state.fuel.fxRateToBase;
    ...
    const tList = getEl('tolls-list'); if(tList) tList.innerHTML = state.fuel.tollsList.map(t => `<div class="list-item"><div class="list-item-main">${t.name}</div><div class="list-item-values">${(t.amount * t.fxRate).toFixed(2)}</div><button class="btn btn-outline btn-sm remove-toll" data-id="${t.id}">×</button></div>`).join('');
    const rList = getEl('refuel-list'); if(rList) rList.innerHTML = state.fuel.refuelList.map(t => `<div class="list-item"><div class="list-item-main">${t.name}</div><div class="list-item-values">${(t.amount * t.fxRate).toFixed(2)}</div><button class="btn btn-outline btn-sm remove-refuel" data-id="${t.id}">×</button></div>`).join('');
};

const renderRates = () => {
    const list = getEl('rates-list'); if(!list) return;
    list.innerHTML = Object.keys(state.currency.manualRates).filter(k => k !== state.trip.baseCurrency).map(code => `<div class="form-group grid" style="grid-template-columns:1fr 2fr; align-items:center;"><label>1 ${code} =</label><input type="text" class="rate-input" data-code="${code}" value="${state.currency.manualRates[code]}"></div>`).join('');
};
...
const renderJournal = () => {
    ...
    let html = ''; for (let h = 0; h < 24; h++) {
        const key = `${state.ui.currentMonth}-${state.ui.currentWeek}-${state.ui.currentDay}-${h}`;
        html += `<div class="journal-entry"><span class="journal-hour">${h}:00</span><textarea class="journal-textarea" data-key="${key}" oninput="window.saveJournalEntry(this)">${state.journal.entries[key] || ''}</textarea></div>`;
    }
...
const renderCalendar = () => {
    ...
    for (let d = 1; d <= days; d++) {
        ...
        html += `<div class="cal-day ${isSel ? 'selected' : ''}" onclick="window.setJournalDate(${d})">${d}</div>`;
    }
...
const renderChecklist = () => {
    ...
    const grps = [...new Set(state.checklist.map(i => i.group))];
    cont.innerHTML = grps.map(g => `<div class="card" style="padding:1rem;"><strong>${g}</strong>${state.checklist.filter(i => i.group === g).map(i => `<div class="list-item"><label><input type="checkbox" class="check-toggle" data-id="${i.id}" ${i.checked ? 'checked' : ''}> ${i.text}</label><button class="btn btn-outline btn-sm remove-check" data-id="${i.id}">×</button></div>`).join('')}</div>`).join('');
...
window.saveJournalEntry = (el) => { state.journal.entries[el.dataset.key] = el.value; save(); };
...
document.addEventListener('click', (e) => {
    ...
    if (t.closest('.nav-btn')) {
        ...
        t.closest('.nav-btn').classList.add('active');
    }
    if (t.id === 'btn-add-exp') {
        const title = getEl('exp-title').value, amt = parseVal(getEl('exp-amount').value);
        if(title && amt) {
            state.expenses.push({ id: Date.now(), title, amountInBase: amt * getExchangeRate(getEl('exp-currency').value), date: getEl('exp-date').value, category: getEl('exp-cat').value });
            save(); render();
        }
    }
    ...
    if (t.id === 'btn-add-refuel') {
        const name = getEl('refuel-name').value, amt = parseVal(getEl('refuel-amount').value);
        if(name && amt) { state.fuel.refuelList.push({ id: Date.now(), name, amount: amt, fxRate: getExchangeRate(getEl('refuel-currency').value), date: getEl('refuel-date').value }); save(); render(); }
    }
    if (t.id === 'btn-add-toll') {
        const name = getEl('toll-name').value, amt = parseVal(getEl('toll-amount').value);
        if(name && amt) { state.fuel.tollsList.push({ id: Date.now(), name, amount: amt, fxRate: getExchangeRate(getEl('toll-currency').value), date: getEl('toll-date').value }); save(); render(); }
    }
    ...
    if (t.id === 'btn-gen-pdf') generatePDF();
    if (t.id === 'btn-reset') { if(confirm(i18n[state.settings.lang].confirm_reset)) { localStorage.removeItem(STORAGE_KEY); location.reload(); } }
    if (t.id === 'theme-btn') { state.settings.theme = state.settings.theme === 'light' ? 'dark' : 'light'; save(); render(); }
    if (t.id === 'btn-add-check') {
        const text = getEl('new-check-text').value;
        if(text) { state.checklist.push({ id: Date.now(), text, checked: false, group: getEl('new-check-group').value }); save(); renderChecklist(); getEl('new-check-text').value = ''; }
    }
...
document.addEventListener('input', (e) => {
    if (['trip-name', 'trip-start', 'trip-end', 'trip-total', 'trip-currency'].includes(e.target.id)) {
        ...
        state.trip.baseCurrency = getEl('trip-currency').value; save(); render();
    }
});
