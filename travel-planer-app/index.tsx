...
const generatePDF = () => {
    ...
    if (incBudget) {
        ...
        <table>
            ...
            <tbody>${state.categories.map((c: any) => `<tr><td>${c.name}</td><td>${c.plannedAmount.toFixed(2)} ${state.trip.baseCurrency}</td></tr>`).join('')}</tbody>
        </table>`;
    }

    if (incExpenses) {
        const totalEx = state.expenses.reduce((acc: number, e: any) => acc + e.amountInBase, 0);
        html += `
        ...
        <table>
            ...
            <tbody>${state.expenses.map((e: any) => `<tr><td>${e.date}</td><td>${e.title}</td><td>${e.category}</td><td>${e.amount} ${e.currency}</td><td>${e.amountInBase.toFixed(2)}</td></tr>`).join('') || `<tr><td colspan="5" style="text-align:center">${t.pdf_empty_expenses}</td></tr>`}</tbody>
        </table>`;
    }
    ...
    if (incFuel) {
        ...
        const tollsC = (state.fuel.tollsList || []).reduce((acc: number, tl: any) => acc + (tl.amount * tl.fxRate), 0);
        ...
        <table>
            ...
            <tbody>${(state.fuel.tollsList || []).map((tl: any) => `<tr><td>${tl.name}</td><td>${tl.amount.toFixed(2)} ${tl.currency}</td><td>${(tl.amount * tl.fxRate).toFixed(2)}</td></tr>`).join('') || `<tr><td colspan="3" style="text-align:center">${t.pdf_empty_tolls}</td></tr>`}</tbody>
    ...
    if (incChecklist) {
        ...
        groups.forEach((g: any) => {
            ...
            <table>
                ...
                <tbody>${state.checklist.filter((i: any) => i.group === g).map((item: any) => `<tr><td style="text-align:center">${item.checked ? t.pdf_yes : t.pdf_no}</td><td>${item.text}</td></tr>`).join('')}</tbody>
            </table>`;
        });
    }
